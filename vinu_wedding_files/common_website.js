...
var extra_live_stream = 0, extra_guest_party = 0;
...
$(document)
    .ready(function () {
        ...
        $.when(
            $.ajax({
                type: "POST", data: { 'registry_url': regurl }, url: customBaseURL + "/weddingwebsite/index/getPostWeddingConfirmation", success: function (data) {
                    return [data.enablePostwedding, data.enableRsvpmanager];
                }
            }),
            $.ajax({
                type: "POST", data: { 'registry_url': regurl }, url: customBaseURL + "/weddingwebsite/index/checkWebsitePrivacy", success: function (data) {
    ...
    .on("click", '.fa-play-circle', function () {
        jQuery(this).addClass("fa-pause-circle").removeClass("fa-play-circle").attr("src", "https://www.weddingwishlist.com/skin/frontend/default/theme692/images/sound-off.png");
        jQuery('body').find('#mymusic').get(0).play();
    })
    .on("click", '.fa-pause-circle', function () {
        jQuery(this).addClass("fa-play-circle").removeClass("fa-pause-circle").attr("src", "https://www.weddingwishlist.com/skin/frontend/default/theme692/images/sound-on.png");
...
function loadExtraElements() {
    jQuery.ajax({
        type: "GET", url: customBaseURL + "/weddingwebsite/index/loadExtraElements",
...
function checkGuestBookStatus() {
    jQuery.ajax({
        type: "POST", data: { 'regurl': regurl }, url: customBaseURL + "/weddingwebsite/index/wishlist",
...
function showGuestbookContent(json) {
    if ($(".view-guestbook").length > 0) { if (json.show_gb) { $(".view-guestbook").show(); $(".view-guestbook").attr("href", json.gb_href); } else { $(".view-guestbook").hide(); } }
...
function validateSitePassword(content) {
    ...
    jQuery.ajax({
        type: "POST", data: { 'regurl': regurl, 'password': password }, url: customBaseURL + "/weddingwebsite/index/validateSitePassword",
        success: function (data) {
            if (data.result.toLowerCase() == 'success' && data.approved) { loadContentData(content); }
...
function getJsonData() {
    ...
    $.getJSON(content_data_url, function (json) {
        ...
        $.getJSON(customRootBaseUrl + `/weddingwebsite/${siteTheme}/assets/json/content.json`, function (json) { content_data = json; });
        return content_data;
    })
}
...
function loadContentData(postWeddingPurchased) {
    if (postWeddingPurchased) {
        if (((rsvp_manager_enabled == 0) || (rsvp_manager_enabled == null) || (rsvp_manager_enabled == '')) && (window.location.href.split("/").indexOf('weddingwebsite') == -1)) {
