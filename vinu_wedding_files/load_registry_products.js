...
function loadProducts() {
    ...
    jQuery.ajax({
        type: "GET", data: postdata, url: customBaseURL+"/weddingwebsite/index/getWishlistItems/id/" + siteId,
        success: function (data) {
            registrySection = (siteTheme == 'theme25' || siteTheme == 'theme26' || siteTheme == 'theme27') ? getBabyRegistrySection() : getRegistrySection();
            if (content_data.visible_sections.indexOf('gift_registry') > -1 && !QueryString.pwa) {
                $('#gift_registry').removeClass('d-none');
                $('.gift_registry.nav-item').removeClass('d-none');
            }
            // Create registry section
            $('#gift_registry .content-list').html(registrySection);
            // Display carousel for mobile
            $('#how-to-buy-carousel').carousel();
            var json = jQuery.parseJSON(data);
            if (json.result == 'success') {
                url = window.location.href;
                var store_url = url.split(regurl)[0];
                ...
                if (total_registry_products > 0) {
                    if(json.registry_type!=''){
                        $('.how-to-buy').addClass('d-none');
                        $('.contribute-section').removeClass('d-none');
                        if (json.registry_type == 'only charity') {
                            $('.contribute-section .gift-direction-title').html('Make a Donation');
                            $('.contribute-section .contribute-text .browse-buy-contribute').html('The couple will be informed as soon as you donate to one of their chosen causes. Click on the charity you would like to support.');
                            $('#gift_registry .section-content').html('Your blessings and presence at our wedding is all we wish for. If however, you would like to gift us something, we would be deeply humbled if you donate to one of the causes we are passionate about. We look forward to celebrating our big day with you.');
                        } else {
                            jQuery.each(content_data.sections, function (index, section) {
                                if (section.type == 'gift_registry') {
                                    $('#gift_registry .section-content').html(section.content);
                                }
                            });    
                        }
                    } else {
                    ...
                    $('#wishlist-items').html(productsList).css({'display': 'flex', 'flex-wrap': 'wrap', 'justify-content': 'center'}).removeClass('d-none');
                } else {
                    ...
                    $('.how-to-buy').addClass('d-none');
                    $('.contribute-section').addClass('d-none');
                    $('#gift_registry .reg-popup-wrapper').addClass('d-none');
                    $('#gift_registry .section-content').html(`<div class="col-sm-12 text-center">${registryMsg}</div><div class="col-sm-12 text-center buy-gift-wrapper">`
    ...
    jQuery.ajax({
        type: "POST", data: postdata, url: customBaseURL+"/weddingwebsite/index/wishlist",
        success: function (data) {
            var json = jQuery.parseJSON(data);
            ...
            if (json.show_gb) {
                $(".view-guestbook-wrapper").show();
                $(".view-guestbook").attr("href", json.gb_href);
            } else {
                $(".view-guestbook-wrapper").hide();
            }
        }
    });
}
...
function loadProductsMultiSelect() {
    ...
    var postdata = { 'regurl': regurl, 'storeId': storeId };
    jQuery.ajax({
        type: "GET", data: postdata, url: customBaseURL+"/weddingwebsite/index/fetchWishlistItems/id/" + siteId,
        success: function (data) {
            registrySection = (siteTheme == 'theme25' || siteTheme == 'theme26' || siteTheme == 'theme27') ? getBabyRegistrySection() : getRegistrySection();
            if (content_data.visible_sections.indexOf('gift_registry') > -1 && !QueryString.pwa) {
                $('#gift_registry').removeClass('d-none');
            ...
            if (json.result == 'success') {
                ...
                if (total_registry_products > 0) {
                    ...
                    $('#wishlist-items').html(productsList).css({'display': 'flex', 'flex-wrap': 'wrap', 'justify-content': 'center'}).removeClass('d-none');
                    $('body').append(`<div class="sticky-btn-add-cart">
...
function getRegistrySection () {
    ...
    var content  =  '<div class="section-content">Your blessings and presence on our wedding day is all we want. But if you still wish to get us something, please refer to our Wishlist containing gifts we like. You can purchase or contribute towards anything on this list.</div>'+
                    '<div class="reg-popup-wrapper text-center"><span class="what-reg-popup">What is a Gift Registry?</span></div>'+
                    ...
                    '<div class="row how-to-buy-row d-none d-sm-flex text-center">'+
                        '<div class=" col-sm-4">'+
                        '<img src="'+customBaseURL+'/skin/frontend/default/theme692/images/browse-gift-collection-on-registry.gif" alt="select a gift you like from the registry">'+
                        '<div class="gift-direction-title">'+
                            'Buy or Contribute'+
                        '</div>'+
                        '<div class="browse-text"><span class="browse-buy-contribute">Choose your gift. Buy it, or contribute any <br>amount of your choice towards it</span></div>'+
                        '</div>'+
                        '<div class="col-sm-4">'+
                        '<img src="'+customBaseURL+'/skin/frontend/default/theme692/images/buy-or-contribute-towards-gift.gif" alt="buy or contribute towards any gift">'+
                        '<div class="gift-direction-title">'+
                            'Immediate Notification'+
                        '</div>'+
                        '<div class="buy-text"><span class="browse-buy-contribute">The couple is informed as soon as you get them a gift.<br>Other guests will not know of your purchase.</span></div>'+
                        '</div>'+
                        '<div class=" col-sm-4">'+
                        '<img src="'+customBaseURL+'/skin/frontend/default/theme692/images/gift-delivered-to-couple-directly.gif" alt="have the gifts delivered directly to the couple">'+
                        '<div class="gift-direction-title">'+
                            'Direct Delivery'+
                        '</div>'+
                        '<div class="contribute-text"><span class="browse-buy-contribute">Gift &amp; message is shipped directly to couple, unless <br>you choose to have it sent to you</span></div>'+
                        '</div>'+
                    '</div>'+
...
function getBabyRegistrySection () {
    var extraClass = (siteTheme == 'theme24') ? [' item', ' left carousel-control', ' right carousel-control'] : ['', '', ''];
    var content  =  '<div class="section-content">Your blessings and presence on our wedding day is all we want. But if you still wish to get us something, please refer to our Wishlist containing gifts we like. You can purchase or contribute towards anything on this list.</div>'+
                    '<div class="reg-popup-wrapper"><span class="what-reg-popup">What is a Gift Registry?</span></div>'+
                    '<div class="how-to-buy">'+
                    ...
                    '<div class="row how-to-buy-row d-none d-sm-flex">'+
                        ...
                        '<div class="gift-direction-title">'+
                            'Buy or Contribute'+
                        '</div>'+
                        '<div class="browse-text text-left"><span class="browse-buy-contribute">Choose your gift. Buy it, or contribute any amount of your choice towards it.</span></div>'+
                        '</div>'+
                        '<div class="col-sm-4">'+
                        '<img src="'+customBaseURL+'/skin/frontend/default/theme692/images/buy-or-contribute-towards-gift.gif" alt="buy or contribute towards any gift">'+
                        '<div class="gift-direction-title">'+
                        ...
                        '<div class="gift-direction-title">'+
                            'Direct Delivery'+
                        '</div>'+
                        '<div class="contribute-text text-left"><span class="browse-buy-contribute">Your gift is shipped directly to host, unless you ask us to ship to you.</span></div>'+
                        '</div>'+
                    '</div>'+
                    '<div id="how-to-buy-carousel" class="carousel slide d-block d-sm-none" data-ride="carousel">'+
                        '<div class="carousel-inner text-center">'+
                        ...
                        '<div class="carousel-item' + extraClass[0] + '">'+
                            ...
                            'Direct Delivery'+
                            '</div>'+
                            '<div class="contribute-text"><span class="browse-buy-contribute">Your gift is shipped directly to host,<br>unless you ask us to ship to you.</span></div>'+
                        '</div>'+
                        '</div>'+
                        '<a class="carousel-control-prev' + extraClass[1] + '" href="#how-to-buy-carousel" role="button" data-slide="prev">'+
                        '<i class="fa fa-arrow-circle-left" aria-hidden="true"></i>'+
...
function reloadOnCartFailure() {
    ...
    jQuery('#alert-notification-modal .modal-header').html(`<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">×</span><span class="sr-only">Close</span></button>`);
    ...
    if (registryCloseStatus) {
        ...
        if (jQuery('#close-registry').length == 0) {
            var modalContent = '<div class="modal fade registry-close-modal" id="close-registry" role="dialog">'
                    ...
                    + '<button type="button" class="close" data-dismiss="modal">×</button><h5 class="modal-title">Registry is closed</h5></div>'
...
function loadAddToCartAlert () {
    ...
    jQuery.ajax({ type: "GET", url: customBaseURL+"/weddingwebsite/index/getAllCartItems",
    success : function(data) { 
        ...
        if(window.matchMedia("(max-width: 991px)").matches){
           if(jQuery('#guest-cart-icon-mobile .cart-count').length>0){
            ...
            jQuery('.navbar.ww-nav-max-width').append(`
                <div id="guest-cart-icon-mobile" class="guest-cart-icon"><a href="https://stg.weddingwishlist.com/checkout/cart" target="_blank"><i class="fa" style="font-size:24px"></i><span class="cart-count">`+cart_qty+`</span></a></div>
            `);
           }
        }else{
            if(jQuery('.guest-cart-icon .cart-count').length>0){
                ...
                <li class="guest-cart-icon"><a href="https://stg.weddingwishlist.com/checkout/cart" target="_blank"><i class="fa" style="font-size:24px"></i><span class="cart-count">`+cart_qty+`</span></a></li>`);
            }
        }
        hideOverlay(); 
    }
...
function loadBestSellerPopup() {
    ...
    popup += '<div class="modal-header" style="border-bottom: none;' + (siteTheme == 'theme24' ? 'display: inline-block;' : '') + '">';
    popup += '<div class="col-sm-12"><p class="icon-title-font text-center text-uppercase mb-0">Find the perfect match!</p><p class="basic-text-font pt-3 mb-0">Choose the budget that works best for you, and let us show you the most loved gift options. If you want to get together with your friends and buy something really nice, just click on \'Group Gift\'</p></div><button type="button" class="close" data-dismiss="modal" style="position: absolute; right: 10px; padding: 0;">×</button>';
    popup += '</div><div class="modal-body"' + (siteTheme == 'theme24' ? ' style="display: inline-block;width: 100%;"' : '') + '><div class="row pl-3 pr-3">';
    popup += '<div class="' + (siteTheme == 'theme24' ? 'col-xs-6 ' : '') + 'col-sm-6 col-md-4"><a target="_blank" href="/best-selling-wedding-gifts?price=0&sort=category&order=asc"><img src="/skin/frontend/default/theme692/images/1100.jpg" alt="Under 1100" class="w-100' + (siteTheme == 'theme24' ? '" style="padding-bottom: 15px;' : ' pb-4') + '"></a></div>';
    popup += '<div class="' + (siteTheme == 'theme24' ? 'col-xs-6 ' : '') + 'col-sm-6 col-md-4"><a target="_blank" href="/best-selling-wedding-gifts?price=0,1&sort=category&order=asc"><img src="/skin/frontend/default/theme692/images/3100.jpg" alt="Under 1100" class="w-100' + (siteTheme == 'theme24' ? '" style="padding-bottom: 15px;' : ' pb-4') + '"></a></div>';
    popup += '<div class="' + (siteTheme == 'theme24' ? 'col-xs-6 ' : '') + 'col-sm-6 col-md-4"><a target="_blank" href="/best-selling-wedding-gifts?price=0,1&sort=category&order=asc"><img src="/skin/frontend/default/theme692/images/5100.jpg" alt="Under 1100" class="w-100' + (siteTheme == 'theme24' ? '" style="padding-bottom: 15px;' : ' pb-4') + '"></a></div>';
    popup += '<div class="' + (siteTheme == 'theme24' ? 'col-xs-6 ' : '') + 'col-sm-6 col-md-4"><a target="_blank" href="/best-selling-wedding-gifts?price=0,1,2&sort=category&order=asc"><img src="/skin/frontend/default/theme692/images/11000.jpg" alt="Under 1100" class="w-100' + (siteTheme == 'theme24' ? '" style="padding-bottom: 15px;' : ' pb-4') + '"></a></div>';
    popup += '<div class="' + (siteTheme == 'theme24' ? 'col-xs-6 ' : '') + 'col-sm-6 col-md-4"><a target="_blank" href="/best-selling-wedding-gifts?price=5&sort=category&order=asc"><img src="/skin/frontend/default/theme692/images/group-gifts.jpg" alt="Under 1100" class="w-100' + (siteTheme == 'theme24' ? '" style="padding-bottom: 15px;' : ' pb-4') + '"></a></div>';
    popup += '<div class="' + (siteTheme == 'theme24' ? 'col-xs-6 ' : '') + 'col-sm-6 col-md-4"><a target="_blank" href="/browse_gifts"><img src="/skin/frontend/default/theme692/images/browse-gifts.jpg" alt="Under 1100" class="w-100' + (siteTheme == 'theme24' ? '" style="padding-bottom: 15px;' : ' pb-4') + '"></a></div>';
    popup += '</div><div class="no-thanks"><a data-dismiss="modal" class="basic-text-font hover teal-color text-uppercase text-bold-600 float-right pr-3" href="javascript:void(0);">No, Thanks</a></div>';
